  ...
  <div class="col-sm-10 col-sm-offset-1 beta delta">
        <article <?php post_class(); ?>>
            <header>
			  ...
			  <?php if($Pmeta['_'. $pType .'_src'][0] != "") { ?><p class="byline author">Author: <?php echo $Pmeta['_'. $pType .'_src'][0]; ?></p><?php } ?>
            </header>
            <div class="entry-content">
              <?php 
					/* Pulling featured image for author if src is not null */
					if($Pmeta['_'. $pType .'_src'][0] != "") {
					$author_img_url = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'thumbnail' );
						echo "<div style='background: url(".$author_img_url[0].") no-repeat center center;' align='right' class='aut-img'></div>";
					}
			  ?>
              <?php the_content(); ?>
              <div class="row">
